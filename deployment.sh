...
cd backend/TenantPipeline/ || exit # stop execution if cd fails
...
sam.cmd build -t bootstrap-template.yaml --region="$REGION" --config-file samconfig-bootstrap.toml
sam.cmd deploy --config-file samconfig-bootstrap.toml --region="$REGION" --parameter-overrides AdminEmailParameter="$1"
...
cd frontend/Admin || exit # stop execution if cd fails
